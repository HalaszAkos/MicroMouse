...
// put function declarations here:
 ...
 void stop();
 void runPid();
double predict_distance(double sensor_value,double sensor_data[],double distance[], int array_range);
//PID
float speed = 600;
double predicted_distance1;
double predicted_distance2;
double predicted_distance3;
double predicted_distance4;
//SPeed--------------------------
...
float PID_output1 =50 ,PID_output2 = 50; //23000
...
void Cycle_10ms(){
 ...
 if (currentTime - previousTime >= 1000) { // Measure the time in seconds
    ...
    calculatePID(3, speed1, PID_output1);
    calculatePID(3, speed2, PID_output2);
    ...
    SerialBT.print("speed1output");
    SerialBT.print(PID_output1);
    SerialBT.print("speed2output");
    SerialBT.print(PID_output2);
  }
...
void Cycle_500ms(){
  ...
  predicted_distance1=predict_distance(IRSensor1,sensor_data_front1,distance_front1,31);

  //bal oldalra
  predicted_distance2=predict_distance(IRSensor2,sensor_data_side2,distance_side2,30);
  //jobb odalra
   predicted_distance3=predict_distance(IRSensor3,sensor_data_side3,distance_side3,30);
  //jobb elore
  predicted_distance4=predict_distance(IRSensor4,sensor_data_front4,distance_front4,31);
...
void runPid(){
 ...
 /* if (predicted_distance2 < 4) {
    Output1 += 2; // increase PWM for left motor if left sensor is too close to the wall
  }
  if (predicted_distance3 < 4) {
    Output2 += 2; // increase PWM for right motor if right sensor is too close to the wall
  }*/
...
double predict_distance(double sensor_value,double sensor_data[],double distance[], int array_range) {
    ...
    int n = array_range;
    for (int i = 1; i < n; ++i) {
        if (sensor_value <= sensor_data[i - 1] && sensor_value >= sensor_data[i]) {
            double slope = (distance[i] - distance[i - 1]) / (sensor_data[i] - sensor_data[i - 1]);
            return distance[i - 1] + slope * (sensor_value - sensor_data[i - 1]);
    
        }
    } return -1; // Invalid sensor value
}
